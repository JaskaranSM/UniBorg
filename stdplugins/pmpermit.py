...
async def approve_p_m(event):
    ...
    chat = await event.get_chat()
    if Config.NO_P_M_SPAM and event.is_private and not is_approved(chat.id):
...
async def approve_p_m(event):
    ...
    reason = event.pattern_match.group(1)
    chat = await event.get_chat()
    if Config.NO_P_M_SPAM and event.is_private and is_approved(chat.id):
        disapprove(chat.id)
